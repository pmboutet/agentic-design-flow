-- Migration: Fix ambiguous ask_session_id in token access functions
-- The issue is that ask_session_id in the SELECT statement conflicts with
-- the RETURNS TABLE column name. We need to qualify it with the table name.
...
-- ============================================================================
-- Fix: Qualify ask_session_id with table name in the initial SELECT
...
-- ============================================================================
-- Fix: Qualify ask_session_id with table name
...
COMMENT ON FUNCTION public.get_ask_session_by_token(VARCHAR) IS 
  'Returns ASK session data for a valid invite token. Fixed ambiguous column reference by qualifying ask_session_id with table name.';

COMMENT ON FUNCTION public.get_ask_messages_by_token(VARCHAR) IS 
  'Returns messages for an ASK session identified by a valid invite token. Fixed ambiguous column reference by qualifying ask_session_id with table name.';
